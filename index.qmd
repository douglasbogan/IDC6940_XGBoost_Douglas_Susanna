...
#py_install(c("numpy", "matplotlib", "seaborn", "xgboost", "scikit-learn", "plotly", "psutil"))
```
...
learning model like XGBoost predict sale prices using a
transaction-level dataset from a metropolitan market, and which features
most influence those predictions?* While R² reflects how well the model
fits the data, RSME shows how accurately the model predicts the target
...
Feature engineering was then conducted to remove high correlation
...
After forming the tuned model, RSME and R² were re-evaluated against the
