...
this project can delve deeper into specific property features in a
particular geographic area, rather than have those unique features lose
meaning due to the generalization that occurs when evaluating multiple
markets. The goal is a careful and well-documented application of a
machine learning model like XGBoost that produces a reliable predictive
model for that market, along with lessons in data selection,
preprocessing, model training, and hyperparameter tuning that others can
adapt. This project will take a single comprehensive transaction dataset
for a metropolitan area, perform standard preprocessing, train the
XGBoost model, and then implement hyperparameter tuning with a practical
workflow that balances speed and thoroughness wherever possible. The
final deliverable will include a clear analysis of predictive
performance, a discussion of the most influential features, and
practical notes for deploying the XGBoost model in a chosen market.
...
overfitting. Those model features matter when handling real estate data
because transaction tables often include many categorical features and
sparse data. Previous research reinforces the point that XGBoost is
...
reached similar conclusions after standard preprocessing and model
...
XGBoost, or eXtreme Gradient Boosting, is a tree-based ensemble method
that is built on the gradient boosting framework. This is where a
predictive model is created by adding many small decision trees (also
known as “weak learners”), one decision tree at a time. This is done so
that each additional tree can focus on correcting the remaining error
from the trees already created [@ramraj2016experimenting]. Therefore, in
practice, this process is typically described as minimizing an overall
objective that combines a loss term (or how far predictions are from the
actual values) with a regularization term that discourages trees from
becoming overly complex. This process helps control overfitting while
preserving the strong predictive power that is desired from the model in
the first place [@li2021understanding] and [@avanijaa2021prediction]. It
is worth noting that XGBoost is also different from “plain” boosting
implementations. This is because XGBoost was engineered for both speed
and scale on large, structured/tabular datasets. XGBoost uses
parallelized computation and efficient data organization. This reduces
lookup time during decision tree construction, and it includes
sparsity-aware handling, which is useful when dealing with many features
or very sparse features [@ramraj2016experimenting] and
[@chen2016xgboost]. Furthermore, these attributes of XGBoost make it
very distinct from bagging-style ensembles such as random forests.
Random forests build many trees using randomized feature selection and
then combine their outputs. However, XGBoost builds an additive sequence
of trees that, through iterations, will improve the same model fit
[@moreno2025comparative].

![Overview of XGBoost model process](OverviewXGB.png){#fig-overviewXGB
fig-align="center"}

@fig-overviewXGB above illustrates the gradient boosting workflow used
by XGBoost in this study. The model begins with a simple base prediction
and iteratively fits small decision trees to the remaining residual
error. Each tree is added to the ensemble to update predictions, and the
process continues until the stopping criteria are met. The objective
function combines prediction error with regularization, which helps
prevent overly complex trees and improves generalization. This
sequential error‑correction process explains why XGBoost performs well
on structured tabular data such as the Ames dataset.

An overview of the modeling workflow followed throughout this project
can be found in @fig-methods below. 

![Overview of method for study](Method.png){#fig-methods
fig-align="center" width="5in"}

The process begins with data selection and preprocessing, followed by
model training and hyperparameter tuning. This workflow ensures a
consistent, reproducible pipeline from raw data to final model
assessment. These steps are explained in detail below.
...
ensure these values are handled differently depending on whether they
indicate missing values or are a meaningful category. Furthermore, data
stored as an incorrect type will be converted for proper model training.
Feature engineering will be conducted to remove any highly correlated
...
fit. All categorical variables will be converted into a numerical format
...
during model development. Because prior research demonstrates that
XGBoost performance is sensitive to hyperparameter tuning, the modeling
process will include a hyperparameter tuning stage rather than relying
on default settings. Hyperparameter tuning will be performed using
cross‑validated grid search. The search will focus on key parameters
that control ensemble complexity and learning behavior, including the
number of boosting iterations, maximum tree depth, and learning rate.
This tuning strategy follows guidance from the research literature,
which emphasizes prioritizing high‑impact parameters to improve model
performance while maintaining processing efficiency where possible
[@kapoor2021simple].
...
findings in the research papers [@ramraj2016experimenting]. Model
performance will be evaluated using the coefficient of determination
(R²), Mean Absolute Error (MAE), Mean Absolute Percentage Error (MAPE),
and Root Mean Squared Error (RMSE). These metrics are standard in the
housing price prediction literature and allow comparison with prior
studies evaluating XGBoost and related models
[@bentejac2019comparative]; [@sharma2024optimal].
...
The train.csv was chosen as the sole data source to prevent data leaking
...
presented also shows Electrical has an additional category "NA", which
is not defined. A closer look at these columns is necessary to determine
...
In @fig-target above, the target variable SalePrice appears right-skewed
and may benefit from a log transformation. The result of this suggestion
is compared in @fig-log below.
...
Root Mean Squared Error (RMSE). These evaluation metrics were chosen to
reflect our goal of developing a model that can accurately predict sales
price and answer our research question: *How well can a machine learning
model like XGBoost predict real estate sale prices using a
transaction-level dataset from a metropolitan market, and which features
most influence those predictions?*

The metrics used in this project were chosen to evaluate the models both
statistically and for real-world interpretability. Log-scale metrics
assess model performance from the same scale the model was trained on to
ensure fair comparison. On the other hand, U.S. dollar-scale metrics
translate these values into a scale that is easier to interpret.
Together, these provide a comprehensive and defensive evaluation of
model performance. RMSE is shown in both log and dollar scales to show
how accurately the model predicts the target variable, the sales price.
R² reflects how well the model fits the data by explaining the
proportion of variance explained by the transformed model, and thus is
on a log-scale. MAE and MAPE are both evaluated in dollar scale for
interpretability. MAE calculates the average error in terms of the
original scale and is included to complement RMSE, which is more
influenced by outliers. Finally, MAPE is evaluated to demonstrate the
average sales price error as a percentage. Although MAPE will be
sensitive to lower-priced homes, it provides a useful measure of
accuracy when interpreted carefully and in addition to the other
metrics.

[@fig-analysis] below provides an overview of the process performed in
this study.
...
Feature engineering was then conducted to remove the highly correlated
feature GarageArea, as identified in the correlation matrix in
@fig-correlation. GarageArea was chosen over GarageCars simply because
it is more common to discuss garage size in terms of the number of
vehicles that fit inside.
...
# save results as file for slides
...
# save results as file for slides
joblib.dump(Xtest_encoded, "Xtest_encoded.pkl")
print("Saved Xtest_encoded to Xtest_encoded.pkl")
```
...
tested with default hyperparameters. The baseline results for the
evaluation metrics against the testing portion of the split data can be
found in @tbl-baseline below.
...
The low value in RMSE indicates the model's strength in predicting sales
price, having only an average discrepancy between actual and predicted
values of \$21,986 for the larger errors. The absolute error on an
average home varies by about \$15,577, according to the MAE, with a
9.46% error shown with MAPE. Additionally, these results indicate that
the model already explains 90% of the variance for the target variable.

#### Hyperparameter Tuning

Although currently performing adequately, hyperparameter tuning could
improve the results of the evaluation metrics. GridSearchCV was used to
evaluate several parameters that can be further examined in the code
block below.
...
After approximately 30 minutes, the following best parameters were
identified:
...
After forming the tuned model, test-set evaluation metrics were
re-evaluated against the baseline model. The tuned evaluation metrics
can be found in @tbl-tuned below.
...
# convert back to dollars
pred_tuned_dollar = np.exp(pred_tuned)
...
The tuned model shows an improved performance across both log-scale and
original dollar-scale metrics. The improvement in RMSE and R²
demonstrates a better-fitting model. Similarly, the decrease in
dollar-scale RMSE and MAE reflects a lower discrepancy between actual
and predicted values, leading to more accurate predictions shown by the
reduced MAPE.
...
visualization can be found in the graph below.
...
The model performs consistently across most of the price range. A LOWESS
(Locally Weighted Scatterplot Smoothing) trendline is present. The
difference between predicted and actual values is less significant along
the trendline when compared to the difference in the values farther from
the trendline. This points out that as the sales prices approach the
upper end of the distribution, model accuracy decreases. This could be
due to the more complex pricing of higher-end homes.

### Conclusion

This capstone project demonstrates that the XGBoost machine learning
model can produce strong, practical real estate sale price predictions
when applied to a well-prepared tabular housing dataset and evaluated
with standard regression metrics. After addressing a small set of
missing values, correcting data types, applying one‑hot encoding for
categorical variables, and log‑transforming the target to reduce
skewness, the baseline XGBoost model could already achieve high
explanatory power and relatively low error in dollar terms. Once the
baseline model was established, hyperparameter tuning provided a clear,
measurable improvement across the full metric set. Tuning lowered RMSE
and MAE while also reducing percentage error (MAPE) and slightly
increasing R². These metrics indicated a more accurate and
better‑fitting model overall. Feature importance results further support
the XGBoost model’s practical value by highlighting drivers of price.
Lot size, frontage, and above‑ground living area were revealed as some
of the most influential predictors. This aligns with how buyers and
sellers commonly determine value. Regarding implications, these results
suggest that a tuned XGBoost model can serve as a reliable tool for
making valuations, especially for typical homes in the central range of
the distribution. However, the predicted versus actual comparison also
indicates that errors increase toward the high end of sales prices,
where unique property characteristics and scarcity effects are harder to
capture with the available structured features. Moreover, key
limitations include reliance on the provided portion of the Ames housing
competition data (train.csv only), a single modeling approach (XGBoost)
rather than a multi‑model benchmark, and the use of only structured
variables. This leaves potentially valuable unstructured variables
unexplored, such as property images or richer neighborhood context. Even
so, the project’s results are a successful demonstration of an
end‑to‑end, reproducible gradient‑boosted tree model. These results
motivate future additional research. Expanding validation beyond one
market dataset, comparing against alternative models, and incorporating
additional structured and unstructured features to further improve
accuracy could all be explored and subsequently built upon the
successful work laid out in this capstone analysis. 
