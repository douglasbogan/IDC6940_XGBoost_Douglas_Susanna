...
set listing prices, underwrite mortgages, estimate tax assessments,
assist insurance providers, guide investment decisions, and help local
governments in decision-making. Therefore, more accurate price
predictions reduce the chances of overpaying or underpricing a home and
make valuation models and appraisals more useful for companies and
...
expanding the input set beyond basic Multiple Listing Service (MLS)
fields. “Deep learning with XGBoost for real estate appraisal”
[@zhao2019deep] integrates image-based aesthetic scores from
convolutional neural networks with structured attributes and then uses
XGBoost in a stacked model. This added visual quality signals and
improved prediction versus tabular-only baselines. “Understanding the
effects of influential factors on housing prices by combining extreme
gradient boosting and a hedonic price model (XGBoost HPM)”
[@li2021understanding] uses XGBoost to rank predictors drawn from
building records, POIs, night lights, and street images. This showed
that diverse contextual features often surface as top drivers in
...
context and interpretable enough to support real estate decision‑making.
This is why the predictive model chosen will be Extreme Gradient
Boosting (XGBoost), implemented as a gradient‑boosted decision tree
ensemble for regression. This method is well-suited for structured,
tabular datasets [@sharma2024optimal]; [@zaki2022house]. XGBoost was
selected due to its strong observed performance on structured datasets,
efficient handling of sparse inputs, and built‑in mechanisms that help
mitigate overfitting [@chen2016xgboost]. These characteristics make it
particularly suitable for modeling housing prices, where relationships
between predictors and outcomes are often nonlinear and context
dependent. The model will be trained using a squared‑error objective
function, which is appropriate for continuous price prediction and
consistent with prior housing price studies using boosted tree
ensembles.

#### Data Preparation

Data preprocessing will address missing values commonly found in real
estate transaction records. Numerical features with missing values will
be imputed using the median to reduce sensitivity to outliers.
Categorical features with missing values will be filled with an explicit
"None" category to preserve information about structural absence (e.g.,
no garage or fireplace), rather than discarding observations or
converting them into nulls.

All categorical variables will be converted into numerical format using
one‑hot encoding. This approach avoids imposing ordinal structure on
nominal attributes and is compatible with tree‑based ensemble models
such as XGBoost. One‑hot encoding is widely used in applied housing
price prediction studies and supports efficient modeling of sparse
feature spaces [@avanijaa2021prediction].

#### Training, Validation, and Evaluation

The dataset will be divided into training and validation subsets using a
fixed random split. The training subset will be used to fit model
parameters, while the validation subset will be reserved for evaluating
performance during model development. Model performance will be
evaluated using the coefficient of determination (R²) and Root Mean
Squared Error (RMSE). These metrics are standard in the housing price
prediction literature and allow comparison with prior studies evaluating
XGBoost and related models [@bentejac2019comparative];
[@sharma2024optimal].

#### Hyperparameter Tuning

Because prior research demonstrates that XGBoost performance is
sensitive to hyperparameter choices, the modeling process will include a
...
maintaining computational efficiency [@kapoor2021simple].
...
influential predictors of housing prices. These rankings will support
interpretation and comparison with findings in the research papers,
consistent with prior XGBoost studies [@ramraj2016experimenting].
...
“Id”), detailed in [@tbl-variables] below, as defined from the original
Kaggle competition data_description.txt file located on
...
#| message: false
#| warning: false
# library to run python
library(reticulate)
py_require(c("pandas", "numpy", "matplotlib", "seaborn"))
pd = import("pandas")
np = import("numpy")
...
::: {#tbl-variables layout-ncol="1"}
+---------------+------------------------------------------------------+
| **Name**      | **Description**                                      |
+:=============:+:====================================================:+
| MSSubClass    | Type of dwelling, as a code (PUD: Planned Unit       |
|               | Development) (20=1 story 1946-recent, 30=1 story     |
| (stored as    | 1945 or older, 40=1 story w/finished attic, 45=1.5   |
| int64)        | story unfinished, 50=1.5 story finished, 60=2 story  |
|               | 1946-recent, 70=2 story 1945 or older, 75=2.5 story, |
|               | 80=Split/multi-level, 85=Split foyer, 90=Duplex,     |
|               | 120=1 story PUD 1946-recent, 160=2 story PUD         |
|               | 1946-recent, 180=Multilevel PUD including            |
|               | split-level/foyer, 190=2 family conversion)          |
+---------------+------------------------------------------------------+
| MSZoning      | General zoning classification (C=Commercial,         |
|               | FV=Floating village residential, RH=Residential      |
|               | high-density, RL=Residential low-density,            |
|               | RM=Residential medium density)                       |
+---------------+------------------------------------------------------+
| Street        | Type of road access (Grvl=Gravel, Pave=Paved)        |
+---------------+------------------------------------------------------+
| Alley         | Type of alley access (Grvl=Gravel, Pave=Paved, NA=   |
|               | None)                                                |
+---------------+------------------------------------------------------+
| LotShape      | Property shape (Reg=Regular, IR1=Slightly irregular, |
|               | IR2=Moderately irregular, IR3=Irregular)             |
+---------------+------------------------------------------------------+
| LandContour   | Flatness of the property (Lvl=Near flat/level,       |
|               | Bnk=Banked - Quick & significant rise from street to |
|               | building, HLS=Hillside - Significant slope side to   |
|               | side, Low=Depression)                                |
+---------------+------------------------------------------------------+
| Utilities     | Utilities available (AllPub=Public electric & gas &  |
|               | water & sewer, NoSeWa=Electric & gas)                |
+---------------+------------------------------------------------------+
| LotConfig     | Lot configuration (Inside, Corner,                   |
|               | CulDSac=Cul-de-sac, FR2=Frontage on 2 sides,         |
|               | FR3=Frontage on 3 sides)                             |
+---------------+------------------------------------------------------+
| LandSlope     | Slope of property (Gtl=Gentle, Mod=Moderate,         |
|               | Sev=Severe)                                          |
+---------------+------------------------------------------------------+
| Neighborhood  | Physical location in Ames city limit                 |
|               | (Blmngtn=Bloomington Heights, Blueste=Bluestem,      |
|               | BrDale=Briardale, BrkSide=Brookside, ClearCr=Clear   |
|               | Creek, CollgCr=College Creek, Crawfor=Crawford,      |
|               | Edwards, Gilbert, IDOTRR=Iowa DOT & rail road,       |
...
|               | NWAmes=Northwest Ames, OldTown=Old Town, SWISU=SW of |
|               | Iowa State University, Sawyer, SawyerW=Sawyer West,  |
|               | Somerst=Somerset, StoneBr=Stone Brook,               |
|               | Timber=Timberland, Veenker)                          |
+---------------+------------------------------------------------------+
| Condition1    | Proximity to condition (RR: Railroad)                |
|               | (Artery=Adjacent to arterial street, Feedr=Adjacent  |
| Condition2    | to feeder street, Norm=Normal, RRNn= Within 200 feet |
| (if more than | of North-South RR, RRAn=Adjacent to North-South RR,  |
| 1)            | PosN=Near positive off-site feature (park,           |
|               | greenbelt, etc.), PosA=Adjacent to positive off-site |
|               | feature, RRNe=Within 200 feet of East-West RR,       |
|               | RRAe=Adjacent to East-West RR)                       |
+---------------+------------------------------------------------------+
| BldgType      | Dwelling type (1Fam=1 family detached, 2FmCon=2      |
|               | family conversion from 1 family, Duplx=Duplex,       |
|               | TwnhsE=Townhouse end unit, TwnhsI=Townhouse inside   |
|               | unit)                                                |
+---------------+------------------------------------------------------+
| HouseStyle    | Style of dwelling (1Story=1 story, 1.5Fin=1.5 story, |
|               | 2nd level finished, 1.5Unf=1.5 story, 2nd level      |
|               | unfinished, 2Story=2 story, 2.5Fin=2.5 story, 2nd    |
|               | level finished, 2.5Unf=2.5 story, 2nd level          |
|               | unfinished, SFoyer=Split foyer, SLvl=Split level)    |
+---------------+------------------------------------------------------+
| RoofStyle     | Roof style (Flat, Gable, Gambrel=Gabriel (Barn),     |
|               | Hip, Mansard, Shed)                                  |
+---------------+------------------------------------------------------+
| RoofMatl      | Roof material (ClyTile=Clay/Tile, CompShg=Standard   |
|               | (composite) shingle, Membran=Membrane, Metal, Roll,  |
|               | Tar&Grv=Gravel & tar, WdShake=Wood shakes,           |
|               | WdShngl=Wood shingles)                               |
+---------------+------------------------------------------------------+
| Exterior1st   | Exterior material (AsbShng=Asbestos shingles,        |
|               | AsphShn=Asphalt shingles, BrkComm=Brick common,      |
| Exterior2nd   | BrkFace=Brick face, CBlock=Cinder block,             |
| (if more than | CemntBd=Cement board, HdBoard=Hard board,            |
| 1)            | ImStucc=Imitation stucco, MetalSd=Metal siding,      |
|               | Other, Plywood, Stone, Stucco, VinylSd=Vinyl siding, |
|               | Wd Sdng=Wood siding, WdShing=Wood shingles)          |
+---------------+------------------------------------------------------+
| MasVnrType    | Masonry veneer type (BrkCmn=Brick common,            |
|               | BrkFace=Brick face, None, Stone)                     |
+---------------+------------------------------------------------------+
| ExterQual     | Exterior material quality (Ex=Excellent, Gd=Good,    |
...
|               | CBlock=Cinder block, PConc=Poured concrete, Slab,    |
|               | Stone, Wood)                                         |
+---------------+------------------------------------------------------+
| BsmtQual      | Evaluation of basement height (Ex=Excellent - 100+   |
|               | inches, Gd=Good - 90-99 inches, TA=Typical - 80-89   |
|               | inches, Fa=Fair - 70-79 inches, NA=No basement)      |
+---------------+------------------------------------------------------+
| BsmtCond      | General condition of the basement (Gd=Good,          |
|               | TA=Typical - slight dampness allowed, Fa=Fair -      |
|               | dampnes s/cracking/settling, Po=Poor - severe cracki |
|               | ng/settling/wetness, NA=No basement)                 |
+---------------+------------------------------------------------------+
| BsmtExposure  | Exposure of walkout/garden level basement walls      |
|               | (split levels/foyers typically score average or      |
|               | above) (Gd=Good, Av=Average, Mn=Minimum, No=None,    |
|               | NA=No basement)                                      |
+---------------+------------------------------------------------------+
| BsmtFinType1  | Quality of basement finished area (GLQ=Good living   |
|               | quarters, ALQ=Average living quarters, BLQ=Below     |
| BsmtFinType2  | average living quarters, Rec=Average rec room,       |
| (if more than | LwQ=Low quality, Unf=Unfinshed, NA=No basement)      |
| 1 type)       |                                                      |
+---------------+------------------------------------------------------+
| Heating       | Type of heating (Floor=Floor furnace, GasA=Gas       |
|               | forced warm air furnace, GasW=Gas hot water or steam |
|               | heat, Grav=Gravity furnace, OthW=Hot water/steam     |
|               | heat other than gas, Wall=Wall furnace)              |
+---------------+------------------------------------------------------+
| HeatingQC     | Heating quality & condition (Ex=Excellent, Gd=Good,  |
|               | TA=Average/Typical, Fa=Fair, Po=Poor)                |
+---------------+------------------------------------------------------+
| CentralAir    | Central air conditioning (N=No, Y=Yes)               |
+---------------+------------------------------------------------------+
| Electrical    | Electrical system (SBrkr=Standard circuit breakers & |
|               | Romex, FuseA=Average - Fuse box over 60 AMP & all    |
|               | Romex, FuseF= Fair - 60 AMP fuse box & mostly Romex, |
|               | FuseP=Poor - 60 AMP fuse box & mostly knob & tube,   |
|               | Mix=Mixed)                                           |
+---------------+------------------------------------------------------+
| KitchenQual   | Kitchen quality (Ex=Excellent, Gd=Good,              |
|               | TA=Average/Typical, Fa=Fair)                         |
+---------------+------------------------------------------------------+
| Functional    | Home functionality rating (assume typical unless     |
|               | deductions warranted) (Typ=Typical, Min1=Minor       |
|               | deductions 1, Min2=Minor 2, Mod=Moderate deductions, |
|               | Maj1=Major deductions 1, Maj2=Major 2, Sev=Severely  |
|               | damaged)                                             |
+---------------+------------------------------------------------------+
| FireplaceQu   | Fireplace quality (Ex=Excellent - Exceptional        |
|               | masonry fireplace, Gd=Good - Masonry fireplace in    |
|               | main level, TA=Average - Prefabricated fireplace in  |
|               | main living area/Masonry fireplace in basement,      |
|               | Fa=Fair - Prefabricated fireplace in basement,       |
|               | Po=Poor - Ben Franklin stove, NA=No fireplace)       |
+---------------+------------------------------------------------------+
| GarageType    | Garage type (2Types=More than 1 type,                |
|               | Attchd=Attached to home, Basment=Basement,           |
|               | BuiltIn=Built-In (part of house - typically has room |
|               | above garage), CarPort=Car Port, Detchd=Detached     |
|               | from home, NA=No garage)                             |
+---------------+------------------------------------------------------+
| GarageFinish  | Garage interior (Fin=Finished, RFn=Rough finished,   |
|               | Unf=Unfinished, NA=No garage)                        |
+---------------+------------------------------------------------------+
| GarageQual    | Garage quality (Ex=Excellent, Gd=Good,               |
|               | TA=Typical/Average, Fa=Fair, Po=Poor, NA=No garage)  |
+---------------+------------------------------------------------------+
| GarageCond    | Garage condition (Ex=Excellent, Gd=Good,             |
|               | TA=Typical/Average, Fa=Fair, Po=Poor, NA=No garage)  |
+---------------+------------------------------------------------------+
| PavedDrive    | Driveway type (Y=Paved, P=Partial pavement,          |
|               | N=Dirt/Gravel)                                       |
+---------------+------------------------------------------------------+
| PoolQC        | Pool quality (Ex=Excellent, Gd=Good, Fa=Fair, NA=No  |
|               | pool)                                                |
+---------------+------------------------------------------------------+
| Fence         | Fence quality (GdPrv=Good privacy, MnPrv=Minimum     |
|               | privacy, GdWo=Good wood, MnWw=Minimum wood/wire,     |
|               | NA=No fence)                                         |
+---------------+------------------------------------------------------+
| MiscFeature   | Miscellaneous feature not covered in other           |
|               | categories (Gar2=2nd Garage not described in garage  |
|               | section, Othr=Other, Shed=Shed more than 100 square  |
|               | feet, TenC=Tennis court, NA=None)                    |
+---------------+------------------------------------------------------+
| SaleType      | Type of sale (WD=Warranty Deed, conventional,        |
|               | CWD=Warranty Deed, cash, New=New construction home,  |
|               | COD=Court Officer Deed/Estate, Con=Contract 15%      |
|               | down, regular terms, ConLw=Contract low down payment |
|               | & low interest, ConLI=Contract low interest,         |
|               | ConLD=Contract low down, Oth=Other)                  |
+---------------+------------------------------------------------------+
| SaleCondition | Condition of sale (Normal, Abnorml=Abnormal - trade, |
|               | foreclosure, short sale, AdjLand=Adjoining land      |
|               | purchase, Alloca=Allocation - 2 linked properties    |
|               | w/separate deeds, typically condo & garage,          |
|               | Family=Sale between family members, Partial=Home not |
|               | yet completed at last assessment, associated w/new   |
|               | homes)                                               |
+---------------+------------------------------------------------------+
...
| **Name**      | **Description**                                      |
+:=============:+:====================================================:+
| LotFrontage   | Distance (linear feet) of street connected to the    |
|               | property                                             |
| (stored as    |                                                      |
| object)       |                                                      |
+---------------+------------------------------------------------------+
...
+---------------+------------------------------------------------------+
| OverallQual   | Overall material & finish quality ratin (10=Very     |
|               | excellent, 9=Excellent, 8=Very good, 7=Good, 6=Above |
|               | average, 5=Average, 4=Below average, 3=Fair, 2=Poor, |
|               | 1=Very poor)                                         |
+---------------+------------------------------------------------------+
| OverallCond   | Overall condition rating (1=Very poor to 10=Very     |
|               | excellent)                                           |
+---------------+------------------------------------------------------+
| YearBuilt     | Original construction year (YYYY)                    |
+---------------+------------------------------------------------------+
| YearRemodAdd  | Remodel year (same as construction date if none)     |
|               | (YYYY)                                               |
+---------------+------------------------------------------------------+
...
| object)       |                                                      |
+---------------+------------------------------------------------------+
| BsmtFinSF1    | Finished basement area (square feet)                 |
|               |                                                      |
| BsmtFinSF2    |                                                      |
| (if type2)    |                                                      |
+---------------+------------------------------------------------------+
...
+---------------+------------------------------------------------------+
| 1stFlrSF      | Area of floor level (square feet)                    |
| (1st)         |                                                      |
|               |                                                      |
| 2ndFlrSF      |                                                      |
| (2nd)         |                                                      |
+---------------+------------------------------------------------------+
| LowQualFinSF  | Low quality finished area, all floors (square feet)  |
+---------------+------------------------------------------------------+
...
+---------------+------------------------------------------------------+
| TotRmsAbvGrd  | Number of rooms above ground (not including          |
|               | bathrooms)                                           |
+---------------+------------------------------------------------------+
...
+---------------+------------------------------------------------------+
| 3SsnPorch     | 3-season porch area (square feet)                    |
+---------------+------------------------------------------------------+
| ScreenPorch   | Screen porch area (square feet)                      |
+---------------+------------------------------------------------------+
| PoolArea      | Pool area (square feet)                              |
+---------------+------------------------------------------------------+
| MiscVal       | Value of miscellaneous feature (\$)                  |
+---------------+------------------------------------------------------+
| MoSold        | Month of sale (MM)                                   |
+---------------+------------------------------------------------------+
| YrSold        | Year of sale (YYYY)                                  |
+---------------+------------------------------------------------------+
| SalePrice     | The sale price (\$), target variable                 |
+---------------+------------------------------------------------------+
...
right-skewed, and may benefit from log transformation. The results of
...
In the correlation matrix, values over 0.85 are annotated for easy
identification. This only incorporates the correlation between
