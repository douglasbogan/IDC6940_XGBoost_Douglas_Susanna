...
Our Literature Review determined the following in regards to utilizing the XGBoost Model for our dataset:

-   XGBoost is well-suited to tabular housing data, but performance depends on careful tuning [@kapoor2021simple]
-   Housing studies consistently show XGBoost improves prediction accuracy and supports feature importance interpretation [@sharma2024optimal]
-   Feature design and validation choices are critical. A single-market focus helps local insight, but sampling must protect rare observations [@zaki2022house]
-   Deployment calls for interpretability and fairness awareness, including feature rankings and bias checks when using the model in practice [@zhang2022research]

## Methods

**Model:** eXtreme Gradient Boosting (XGBoost) [@verma2024exploring]
...
-   Many categorical variables with a category "NA", so NA filter was set to False to not automatically assign these as missing data
    -   Reassigned true null values for 4 features
        ...
        -   LotFrontage (259) and MasVnrArea (8) became 0
...
-   GarageArea and GarageCars highly correlated, dropped GarageArea
